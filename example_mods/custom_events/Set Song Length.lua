    ...
    if n == 'Set Song Length' then
        ...
        elseif tonumber(funnyyy) > 0 then
            ...
            runHaxeCode([[
				...
				game.modchartTweens.set(tweenTag, FlxTween.tween(game, {needSongLength: targetValue}, duration, {ease: FlxEase.]]..fnuuyyy..[[,
                    onComplete: function(twn:FlxTween) {
                        game.callOnLuas('onTweenCompleted', [tweenTag]);
                        game.modchartTweens.remove(tweenTag);
                    },
                    onUpdate: function(_) {
                        game.callOnLuas('onFakeoutUpdate', [game.songLength]);
                    }
                }));
            ]])
        end
...
function onFakeoutUpdate(length)
    setSongLength(length, false)
end