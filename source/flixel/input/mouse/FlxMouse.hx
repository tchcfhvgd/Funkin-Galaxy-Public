...
class FlxMouse extends FlxPointer implements IFlxInputManager
{
	...
	public var actualJustMoved(get, never):Bool;
	...
	var actualPos:{var now:Vector2; var prev:Vector2;} = {prev: {x: 0, y: 0}, now: {x: 0, y: 0}};
	...
	function update():Void
	{
		...
		if (visible)
		{
			actualPos.prev.x = actualPos.now.x;
			actualPos.prev.y = actualPos.now.y;
			actualPos.now.x = cursorContainer.x = FlxG.game.mouseX;
			actualPos.now.y = cursorContainer.y = FlxG.game.mouseY;
		}
		#end

		if (actualJustMoved) 
			moved(actualPos.now, {x: getWorldPosition().x, y: getWorldPosition().y});
	...
	public function moved(screenPos:Vector2, worldPos:Vector2) {
		if ((visible && !useSystemCursor) && Type.getClass(FlxG.state) == PlayState) @:privateAccess {
			var blueEffect = new ColorTransform();
        	blueEffect.color = FlxColor.fromRGB(50, 70, 210).to24Bit();

			var mouseData:BitmapData = MouseCursors.getCursorBitmapData(MouseCursors.currentCursor);
            mouseData.colorTransform(mouseData.rect, blueEffect);

			var mySprite = new Bitmap(mouseData);
			mySprite.x = (screenPos.x + MouseCursors.offsets.get(MouseCursors.currentCursor)[0]);
			mySprite.y = (screenPos.y + MouseCursors.offsets.get(MouseCursors.currentCursor)[1]);
			mySprite.alpha = 0.5;
			FlxG.addChildBelowMouse(mySprite);
			flixel.tweens.FlxTween.tween(mySprite, {alpha: 0}, 0.2, {onComplete: function(_){
				mySprite = FlxDestroyUtil.removeChild(FlxG.stage, mySprite);
			}});
		}
	}
	...
	inline function get_actualJustMoved():Bool
		return actualPos.prev.x != actualPos.now.x || actualPos.prev.y != actualPos.now.y;
