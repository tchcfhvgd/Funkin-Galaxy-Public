...
class EditorPlayState extends MusicBeatState
{
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		if(key > -1)
		{
			...
			if(spr != null)
			{
				...
				spr.resetAnim = 0;
				if (spr.cover != null ) {
					if (spr.cover.animation.curAnim.name != 'end') spr.cover.visible = false;
		...
		if (!note.wasGoodHit)
		{
			...
			playerStrums.forEach(function(spr:StrumNote)
			{
				if (Math.abs(note.noteData) == spr.ID)
				{
					...
					spr.coverLogic(note);
					if (note.tail.length > 0) spr.heldNote = note;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var daRating:String = Scoring.judgeNote(noteDiff, ClientPrefs.scoreMode == "Legacy" ? LEGACY : PBOT1);

		if(daRating.toLowerCase() == 'sick' && !note.noteSplashDisabled)
		{
			spawnNoteSplashOnNote(note);
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			new FlxTimer().start(Conductor.crochet / 1000 / 2, function(_)
			{
