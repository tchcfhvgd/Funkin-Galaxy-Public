...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		["The Star Moment", "pop them up"],
		["The Star Moment The Sequel", "make them enter"]
	];
	...
	**/
	...
	override function create()
	{
		...
		leftIcon.iconScale = leftIcon.scale.x;
		rightIcon.iconScale = rightIcon.scale.x;
		...
		var achieveID:Int = Achievements.getAchievementIndex('debug');
		if(!Achievements.isAchievementUnlocked(Achievements.achievementsStuff[achieveID][2]))
		{
			Achievements.achievementsMap.set(Achievements.achievementsStuff[achieveID][2], true);
			add(new AchievementObject('debug', camAchievement));
			FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
			trace('Giving achievement "debug"');
		}
		#end
	...
	function addSongUI():Void
	{
		...
		{
			PlayState.SONG = Song.parseJSONshit(Json.stringify(FlxG.save.data.autosave, "\t"));
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'Clear notes', function()
			{
				openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){for (sec in 0..._song.sections.length) {
					_song.sections[sec].sectionNotes = [];
				}
				updateGrid();
			}, null,ignoreWarnings));
		...
		var check_flip_health = new FlxUICheckBox(noteSkinInputText.x + noteSkinInputText.width + 10, noteSkinInputText.y, null, null, "Has healthbar flipped", 100);
		check_flip_health.checked = _song.flippedHealth;
		check_flip_health.callback = function()
		{
			_song.flippedHealth = check_flip_health.checked;
		};

		var check_flip_notes = new FlxUICheckBox(noteSplashesInputText.x + noteSplashesInputText.width + 10, noteSplashesInputText.y, null, null, "Has notes flipped", 100);
		check_flip_notes.checked = _song.flippedNotes;
		check_flip_notes.callback = function()
		{
			_song.flippedNotes= check_flip_notes.checked;
		};

		var check_hide_combo = new FlxUICheckBox(noteSplashesInputText.x + noteSplashesInputText.width + 10, (noteSplashesInputText.y - noteSkinInputText.y) + noteSplashesInputText.y , null, null, "Hide Combo", 100);
		check_hide_combo.checked = _song.hideCombo;
		check_hide_combo.callback = function()
		{
			_song.hideCombo = check_hide_combo.checked;
		};
		...
		tab_group_song.add(check_flip_health);
		tab_group_song.add(check_flip_notes);
		tab_group_song.add(check_hide_combo);
	...
	function addSectionUI():Void
	{
		...
		var copyButton:FlxButton = new FlxButton(10, 190, "Copy Section", function()
		{
			...
			for (i in 0..._song.sections[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.sections[curSec].sectionNotes[i];
			...
			for (note in notesCopied)
			{
				...
				else
				{
					if(check_notesSec.checked)
					{
						...
						_song.sections[curSec].sectionNotes.push(copiedNote);
					}
				}
			}
			updateGrid();
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Clear", function()
		{
			if(check_notesSec.checked)
			{
				_song.sections[curSec].sectionNotes = [];
			}
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			for (i in 0..._song.sections[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.sections[curSec].sectionNotes[i];
				note[1] = (note[1] + 4) % 8;
				_song.sections[curSec].sectionNotes[i] = note;
			}
			updateGrid();
		});
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "Copy last section", function()
		{
			...
			for (note in _song.sections[daSec - value].sectionNotes)
			{
				var strum = note[0] + Conductor.stepCrochet * (getSectionBeats(daSec) * 4 * value);


				var copiedNote:Array<Dynamic> = [strum, note[1], note[2], note[3]];
				_song.sections[daSec].sectionNotes.push(copiedNote);
			}
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			for (note in _song.sections[curSec].sectionNotes)
			{
				var boob = note[1];
				if (boob>3){
					boob -= 4;
			...
			for (i in duetNotes){
			_song.sections[curSec].sectionNotes.push(i);
	...
	var UI_songCharter:FlxUIInputText;
	...
	function addCreditsUI() {
		...
		UI_songComposer = new FlxUIInputText(5, 15, 150, _song.songComposer == null ? "Composer" : _song.songComposer);
		blockPressWhileTypingOn.push(UI_songComposer);
	
		UI_songCharter = new FlxUIInputText(UI_songComposer.x, UI_songComposer.y + UI_songComposer.height + 20, 150, _song.songCharter == null ? "Charter" : _song.songCharter);
		blockPressWhileTypingOn.push(UI_songCharter);

		UI_songArtist = new FlxUIInputText(UI_songCharter.x, UI_songCharter.y + UI_songCharter.height + 20, 150, _song.songArtist == null ? "Artist" : _song.songArtist);
		blockPressWhileTypingOn.push(UI_songArtist);
	
		tab_group_credits.add(UI_songComposer);
		tab_group_credits.add(new FlxText(UI_songComposer.x, UI_songComposer.y - 15, 0, 'Song Composer:'));
		tab_group_credits.add(UI_songCharter);
		tab_group_credits.add(new FlxText(UI_songCharter.x, UI_songCharter.y - 15, 0, 'Song Charter:'));
		tab_group_credits.add(UI_songArtist);
		tab_group_credits.add(new FlxText(UI_songArtist.x, UI_songArtist.y - 15, 0, 'Song Artist:'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				_song.sections[curSec].sectionBeats = nums.value;
				reloadGridLayer();
			}
	...
	override function update(elapsed:Float)
	{
		...
		_song.songComposer = UI_songComposer.text;
		_song.songCharter = UI_songCharter.text;
		_song.songArtist = UI_songArtist.text;
		...
		curRenderedSustains.forEachAlive(function(sustain:FlxSprite)
		{
			var note:Note = sustain.data.note;
			if(note.strumTime <= Conductor.songPosition) {
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.sections[curSec].mustHitSection) noteDataToCheck += 4;
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							if(_song.boyfriend == 'gf') { //Easter egg
	...
	*/
	...
	{
		var healthIconP1:String = loadHealthIconFromCharacter(_song.boyfriend);
		var healthIconP2:String = loadHealthIconFromCharacter(_song.opponent);

		if (_song.sections[curSec].mustHitSection)
		{
			leftIcon.changeIcon(healthIconP1);
			rightIcon.changeIcon(healthIconP2);
			if (_song.sections[curSec].gfSection) leftIcon.changeIcon('gf');
		}
		else
		{
	...
	function updateGrid():Void
	{
		...
		for (i in _song.sections[curSec].sectionNotes)
		{
		...
		if(curSec < _song.sections.length-1) {
			for (i in _song.sections[curSec+1].sectionNotes)
			{
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		colorNoteForQuant(note);
		//if(isNextSection) note.y += gridBG.height;
		if(note.y < -150) note.y = -150;
		return note;
	}

	function colorNoteForQuant(note:Note)
	{
		if (note.rgbShader.enabled == false) return;
		var col = 0xFFFFFFFF;
		var col2 =0xFF313131;
		var beat = round(Conductor.bpm * note.strumTime / 1000 / 60 * 48,0);
    	if(beat%(192/4)==0){
    	    col = 0xFFFF0000;
    	    col2 = 0xFF7F0000;
    	}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.sections[curSec].mustHitSection) noteDataToCheck += 4;
			for (i in _song.sections[curSec].sectionNotes)
			{
		...
		if(note.noteData > -1) //Normal Notes
		{
			for (i in _song.sections[curSec].sectionNotes)
			{
				if (i[0] == note.strumTime && i[1] == noteDataToCheck)
				{
					...
					_song.sections[curSec].sectionNotes.remove(i);
					break;
				}
			}
		}
	...
	function clearSong():Void
	{
		...
		{
			_song.sections[daSection].sectionNotes = [];
		}
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if(noteData > -1)
		{
			_song.sections[curSec].sectionNotes.push([noteStrum, noteData, noteSus, noteTypeIntMap.get(daType)]);
			curSelectedNote = _song.sections[curSec].sectionNotes[_song.sections[curSec].sectionNotes.length - 1];
		}
		else
		{
		...
		{
			_song.sections[curSec].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, noteTypeIntMap.get(daType)]);
		}
	...
	private function saveEvents()
	{
		...
		var data:String = Json.stringify(eventsSong, "\t");
	...
	function getSectionBeats(?section:Null<Int> = null)
	{
		...
		if(_song.sections[section] != null) val = _song.sections[section].sectionBeats;
