...
class MusicPlayer extends FlxGroup 
{
	...
	override function update(elapsed:Float)
	{
		...
		if (paused && !wasPlaying)
			songTxt.text = instance.songs[FreeplayState.curSelected].songName + ' (PAUSED)';
		else
			songTxt.text = instance.songs[FreeplayState.curSelected].songName;
	...
	public function switchPlayMusic()
	{
		...
		if (playingMusic)
		{
            var leText = "Press SPACE to Pause / Press ESCAPE to Exit / Press {RESET1} to Reset the Song";
            if (ClientPrefs.keyBinds.get("reset")[0] != FlxKey.NONE && ClientPrefs.keyBinds.get("reset")[1] != FlxKey.NONE)
            {
                leText = "Press SPACE to Pause / Press ESCAPE to Exit / Press {RESET1} or {RESET2} to Reset the Song";
                leText = StringTools.replace(leText, "{RESET2}", ClientPrefs.keyBinds.get("reset")[1].toString());
            }
            if (ClientPrefs.keyBinds.get("reset")[0] != FlxKey.NONE && ClientPrefs.keyBinds.get("reset")[1] != FlxKey.NONE)
            {
                leText = "Press SPACE to Pause / Press ESCAPE to Exit / Press {RESET1} to Reset the Song";
			...
			instance.bottomText.text = leText;
			positionSong();
	...
	function positionSong() 
	{
		...
		progressBar.x = songTxt.x + songTxt.width / 2 - 10;
		if (shortName)
		{
